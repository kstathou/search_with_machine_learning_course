...
def search(client, user_query, vector_search=False, top_k=10, index="bbuy_products", sort="_score", sortDir="desc"):
    #### W3: classify the query
    #### W3: create filters and boosts
    # Note: you may also want to modify the `create_query` method above
    # Add vector search
    if vector_search:
        query_obj = create_vector_query(user_query, top_k=top_k)
    else:
...
if __name__ == "__main__":
    ...
    for line in fileinput.input():
        ...
        if query == "Exit":
            break
        search(client=opensearch, user_query=query, index=index_name, vector_search=args.vector)
