...
class DataPrepper:
    ...
    def __log_ltr_query_features(self, query_id, key, query_doc_ids, click_prior_query, no_results, terms_field="_id"):
        ...
        features = [
            ...
            "short_desc_match_phrase",
            "long_desc_match_phrase",
            "salesRankShortTerm",
            ]
        ...
        if response and response['hits']['hits'] is not None and len(response['hits']['hits']) > 0:
            hits = response['hits']['hits']
            ...
            for hit in hits:
                ...
                for feature in features:
                    for log in log_entry:
                        if log["name"] == feature:
                            try:
                                feature_results[feature].append(log['value'])
                            except KeyError:
